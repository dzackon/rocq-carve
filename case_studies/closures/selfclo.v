...
Inductive updn K { t : Type } :
  mlctx K -> nat -> K -> K -> t -> t -> mlctx K-> Prop :=
| updn_t : forall (Δ : mlctx K) (X X' : K) (A A' : t),
    updn K ((X, A) :: Δ) 0 X X' A A' ((X', A') :: Δ)
| updn_n : forall (Δ Δ' : mlctx K ) (n : nat) (X X' Y : K) (A A' B : t),
    updn K Δ n X X' A A' Δ' ->
    updn K ((Y, B) :: Δ) ( S n) X X' A A' ((Y, B) :: Δ').
...
| t_Var: forall (Δ Δ': tenv) (T : ty) (n : nat),
    updn ty  Δ n T T one zero Δ' -> exh hal Δ' -> has_type Δ (var n) T
...
Lemma lookup_hasty: forall η n W  ,
    ...
    ->  updn ty Δ n A A  m m' Δ' ->  has_ty W A.
Proof.
...
Lemma REG_preservation2: forall Δ1 Δ2 Δ ,
    join Δ1 Δ2 Δ ->  forall η, Δ ~~ η  ->  Δ2 ~~ η .
  intros  Δ1 Δ2 Δ H.
  induction H; intros.
  - inv H; constructor.
  - inv H1.
 destruct W.
  constructor.
  now eapply IHmerge. sauto.
  inv H0; sauto.
    Qed.

Corollary  REG_preservation2': forall Δ1 Δ2 Δ η ,
    join Δ1 Δ2 Δ ->   Δ ~~ η  ->  Δ2 ~~ η .   
    sfirstorder use: REG_preservation2.
    Qed.
     
Lemma REG_lookup: forall Δ η, Δ ~~ η -> forall Δ' n m m' T, updn ty Δ n T T m m' Δ' ->  exists W, lookup_venv n W η  /\ W ∈ T.
Proof.
  intros Δ η RR.
  induction RR.
