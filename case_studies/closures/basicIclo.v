...
Definition here T := @ZVAR [] T.
Check abs  (var ZVAR)  : (tm [] (ty_Arrow ty_Unit ty_Unit)
).

Definition lid : (tm [] (ty_Arrow ty_Unit ty_Unit)) := abs  (var ZVAR)  .

Notation "'U'" := ty_Unit .

(* \x \y x y  ~~ abs (abs (app 1 0)*) 
Check abs (abs  (app (var (SVAR ZVAR)) (var ZVAR)))  : tm []
         (ty_Arrow (ty_Arrow U U )
            (ty_Arrow U U )). 


(** ---- Mutual, intrinsically-typed environments and values ---- *)
...
Corollary total:  forall  t (e : tm [] t) ,
