...
Proof.
  ...
  - (* Inductive case: n = S n' *)
    ...
    destruct (join_cons_inv J) as (_ & _ & _ & _ & Jtail).
    ...
    destruct (mult_eq_dec (snd (Δ1 None)) one) as [Heq1|Hneq1];
    destruct (mult_eq_dec (snd (Δ2 None)) one) as [Heq2|Hneq2].

    (* Case: both heads are one (impossible) *)
    + rewrite Heq1, Heq2 in Jhead; inversion Jhead.
    ...
    + split; [now apply join_comm|].
      ...
      split; [destruct x; auto; split; intro Heq;
        [rewrite Heq1 in Jhead; inversion Jhead;
        assert (Hty' := proj1 (Hty None) (eq_sym H7));
        now rewrite Heq01 in Hty'
        | assert (Heq1' := proj2 (neq_one _) Heq);
        rewrite <- Heq1 in Heq1'; now destruct Heq1']|].
    ...
    + split; [assumption|].
      ...
      split; [sauto use: zero_tenv_exh|].
      split; [destruct x;
        [sauto | intros ?; rewrite Heq2 in Jhead; inversion Jhead;
        assert (Hty' := proj1 (Hty None) (eq_sym H7));
        now rewrite Heq02 in Hty']|].
      intro Heq; destruct x;
        [sauto | assert (Heq2' := proj2 (neq_one _) Heq);
        rewrite <- Heq2 in Heq2'; now destruct Heq2'].
