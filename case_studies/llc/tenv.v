...
Proof.
  ...
  destruct (Δ₁ x) as [t1 m1], (Δ₂ x) as [t2 m2], (Δ x) as [t m].
  cbn in *. rewrite H0 in *. inversion H. cbn in *.
...
Property join_cons_inv {n} {Δ₁ Δ₂ Δ : tenv (S n)} :
  join Δ₁ Δ₂ Δ →
  Δ₁ = (scons (fst (Δ None), snd (Δ₁ None)) (λ i, Δ₁ (Some i))) ∧
  Δ₂ = (scons (fst (Δ None), snd (Δ₂ None)) (λ i, Δ₂ (Some i))) ∧
  Δ  = (scons (fst (Δ None), snd (Δ None)) (λ i, Δ (Some i))) ∧
  join (snd (Δ₁ None)) (snd (Δ₂ None)) (snd (Δ None)) ∧
  join (λ i, Δ₁ (Some i)) (λ i, Δ₂ (Some i)) (λ i, Δ (Some i)).
Proof.
  intros * H.
  specialize (H None) as H'. destruct H' as [[E1 E2] H2].
  assert (pair_eta : ∀ {A B} (p : A * B), p = (fst p, snd p)) by sauto.
  repeat split;
    try (apply functional_extensionality; destruct x; rewrite <- E2, <- E1; try apply pair_eta);
    try specialize (H (Some x)); sauto.
Qed.
...
Property upd_some {n} (Δ : tenv n) (fn : fin n) (x y : ty * mult) :
  upd (x .: Δ) (Some fn) y = x .: (upd Δ fn y).
...
Property zero_tenv_exh {n} (Δ : tenv n) : exh hal (zero_tenv Δ).
Proof. intro x; unfold zero_tenv; sauto. Qed.

Property exh_zero_tenv {n} (Δ : tenv n) : exh hal Δ ↔ Δ = zero_tenv Δ.
Proof. split; unfold zero_tenv; intros; [extensionality x; specialize (H x)|rewrite H; intro]; sauto. Qed.
...
Property up_ren_cons {n} (ξ : fin n → fin n) (Δ : tenv n) (x : ty * mult) :
  ren_tfctx (up_ren ξ) (x .: Δ) = x .: (ren_tfctx ξ Δ).
...
Proof.
  intro Hjf. cbn.
  exists (scons (fst h, zero) Δ).
  split.
  - now apply join_family_scons_head.
  - intro x; destruct x; [split; cbn; destruct (Δ f) | split]; sauto.
Qed.
...
Definition var_family {n} (Δ : tenv n) : fin n → tenv n :=
  λ i j, let '(A, m) := Δ j in
         if eq_dec i j then (A, m) else (A, zero).
...
Property var_family_zero {n} (Δ : tenv n) (i j : fin n) :
  i ≠ j → var_family Δ i j = (fst(Δ j), zero).
...
Proof.
  ...
  - intro; contradiction.
  - set (head := (fst (Δ None), zero)).
    set (tail := λ i, Δ (Some i)).
    exists (scons head tail); split.
    + assert (var_family_Some :
        ∀ n (Δ : tenv (S n)) (i j : fin n),
          var_family Δ (Some i) (Some j) = var_family (λ i', Δ (Some i')) i j).
      { intros; unfold var_family.
        destruct (eq_dec i j); destruct (eq_dec (Some i : fin (S n)) (Some j)); sauto. }
      assert (var_family_tail_scons :
        ∀ n (Δ : tenv (S n)),
          (λ i, var_family Δ (Some i)) =
          (λ i, (fst (Δ None), zero) .: var_family (λ i, Δ (Some i)) i)).
      ...
      exact (join_family_scons_head head (IH tail) halz).
    + assert (∀ m, join m zero m) as Hjoin by (destruct m; constructor). (* TODO: use algebra *)
