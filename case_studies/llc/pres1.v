...
Proof.
  ...
  - (* t_App *)
    ...
    destruct (join_cons_inv J1) as (Eq1 & Eq2 & _ & J2 & J3).
    simpl in *; inversion J2; rewrite <- H4, <- H3 in *.
    destruct (IHhas_type1 _ _ _ eq_refl JMeq_refl JMeq_refl b _ _ Eq1) as (? & ? & ->).
    destruct (IHhas_type2 _ _ _ eq_refl JMeq_refl JMeq_refl b _ _ Eq2) as (? & ? & ->).
    eexists; split.
...
Proof.
  ...
  - (* t_App *)
    ...
    destruct (join_cons_inv J1) as (Eq1 & Eq2 & _ & Jhd & Jtl).
    simpl in Jhd, Eq1, Eq2.
    inversion Jhd; rewrite <- H1, <- H3 in *.
    + destruct (join_assoc (join_comm Jtl) J) as [? [J0 J2]].
      apply (typing_ren_sym b) in H1_0.
      admit.
