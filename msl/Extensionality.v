...
Proof. intros. rewrite H; rewrite H0; auto. Qed.

Lemma and_ext': forall (A: Prop) B C D, A = B -> (A -> (C = D)) -> (A /\ C) = (B /\ D).
Proof. intros. subst B. apply prop_ext. intuition; subst; auto. Qed.
...
Proof. intros. apply prop_ext. split; intros. rewrite <- H; auto. rewrite H; auto. Qed.

Lemma existT_ext:
  forall (A: Type) (P: A -> Prop) (x y: A) (Hx: P x) (Hy: P y),
     x = y -> existT _ x Hx = existT _ y Hy.
Proof. intros. subst. rewrite (proof_irr Hx Hy); auto. Qed.
