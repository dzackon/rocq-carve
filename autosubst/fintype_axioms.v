From Autosubst Require Import core core_axioms fintype.
...
Proof. fext. intros [x|]. reflexivity. simpl. reflexivity. Qed.
...
Ltac fsimpl_fext :=
  ...
  | _ => first [ progress (rewrite scons_comp)
               | progress (rewrite scons_eta_id)
               | progress (autorewrite with FunctorInstancesFext)]
  end.
...
Ltac fsimplc_fext :=
  ...
  | [H: context[(?f >> ?g) >> ?h] |- _] => change ((f >> g) >> h) with (f >> (g >> h)) in H (* AsimplComp *)
  ...
  | [H: context[?f >> (?x .: ?g)] |- _] => change (f >> (x .: g)) with g in H
  | [H: context[?x2 .: shift >> ?f] |- _] => change x2 with (f var_zero) in H; rewrite (@scons_eta _ _ f) in H
  | [H: context[?f var_zero .: ?g] |- _] => change g with (shift >> f) in H; rewrite scons_eta in H

  | _ => first [ progress (rewrite scons_comp in *)
               | progress (rewrite scons_eta_id in *)
               | progress (autorewrite with FunctorInstancesFext in *)]
  end.
