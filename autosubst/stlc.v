...
Lemma up_tm_tm {m n_tm : nat} (sigma : fin m -> tm n_tm) :
  fin (S m) -> tm (S n_tm).
Proof. exact (scons (var_tm (S n_tm) var_zero) (funcomp (ren_tm shift) sigma)). Defined.

Lemma up_list_tm_tm (p : nat) {m n_tm : nat} (sigma : fin m -> tm n_tm) :
  fin (plus p m) -> tm (plus p n_tm).
Proof.
...
Fixpoint ext_tm {m_tm n_tm : nat} (sigma_tm : fin m_tm -> tm n_tm)
  ...
  | lam _ s0 s1 =>
      congr_lam (eq_refl s0)
        (ext_tm (up_tm_tm sigma_tm) (up_tm_tm tau_tm) (upExt_tm_tm _ _ Eq_tm) s1)
  end.
...
Fixpoint compRenRen_tm {k_tm l_tm m_tm : nat}
                       ...
                       (Eq_tm : forall x, funcomp zeta_tm xi_tm x = rho_tm x) (s : tm m_tm) {struct s} : 
...
Fixpoint compRenSubst_tm {k_tm l_tm m_tm : nat}
                         ...
                         (Eq_tm : forall x, funcomp tau_tm xi_tm x = theta_tm x) (s : tm m_tm) {struct s} :
...
Proof. exact (compRenRen_tm xi_tm zeta_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (fun s => compRenRen_tm xi_tm zeta_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (compRenSubst_tm xi_tm tau_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (fun s => compRenSubst_tm xi_tm tau_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (compSubstRen_tm sigma_tm zeta_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (fun s => compSubstRen_tm sigma_tm zeta_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (compSubstSubst_tm sigma_tm tau_tm _ (fun n => eq_refl) s). Qed.
...
Proof. exact (fun s => compSubstSubst_tm sigma_tm tau_tm _ (fun n => eq_refl) s). Qed.
...
Lemma rinstInst'_tm {m_tm n_tm : nat} (xi_tm : fin m_tm -> fin n_tm) (s : tm m_tm) :
  ren_tm xi_tm s = subst_tm (funcomp (var_tm n_tm) xi_tm) s.
Proof. exact (rinst_inst_tm xi_tm _ (fun n => eq_refl) s). Qed.

Lemma rinstInst'_tm_pointwise {m_tm n_tm : nat} (xi_tm : fin m_tm -> fin n_tm) :
  pointwise_relation _ eq (ren_tm xi_tm)
    (subst_tm (funcomp (var_tm n_tm) xi_tm)).
Proof. exact (fun s => rinst_inst_tm xi_tm _ (fun n => eq_refl) s). Qed.

Lemma instId'_tm {m_tm : nat} (s : tm m_tm) : subst_tm (var_tm m_tm) s = s.
Proof. exact (idSubst_tm (var_tm m_tm) (fun n => eq_refl) s). Qed.

Lemma instId'_tm_pointwise {m_tm : nat} :
  pointwise_relation _ eq (subst_tm (var_tm m_tm)) id.
Proof. exact (fun s => idSubst_tm (var_tm m_tm) (fun n => eq_refl) s). Qed.
...
Lemma varL'_tm {m_tm n_tm : nat} (sigma_tm : fin m_tm -> tm n_tm) (x : fin m_tm) :
  subst_tm sigma_tm (var_tm m_tm x) = sigma_tm x.
Proof. exact eq_refl. Qed.

Lemma varL'_tm_pointwise {m_tm n_tm : nat} (sigma_tm : fin m_tm -> tm n_tm) :
  pointwise_relation _ eq (funcomp (subst_tm sigma_tm) (var_tm m_tm)) sigma_tm.
Proof. exact (fun x => eq_refl). Qed.

Lemma varLRen'_tm {m_tm n_tm : nat} (xi_tm : fin m_tm -> fin n_tm) (x : fin m_tm) :
  ren_tm xi_tm (var_tm m_tm x) = var_tm n_tm (xi_tm x).
Proof. exact eq_refl. Qed.

Lemma varLRen'_tm_pointwise {m_tm n_tm : nat} (xi_tm : fin m_tm -> fin n_tm) :
...
#[global] Instance Subst_tm  {m_tm n_tm : nat}: (Subst1 _ _ _) := (@subst_tm m_tm n_tm).

#[global] Instance Up_tm_tm  {m n_tm : nat}: (Up_tm _ _) := (@up_tm_tm m n_tm).

#[global] Instance Ren_tm  {m_tm n_tm : nat}: (Ren1 _ _ _) := (@ren_tm m_tm n_tm).

#[global] Instance VarInstance_tm  {n_tm : nat}: (Var _ _) := (@var_tm n_tm).
...
#[global] Instance subst_tm_morphism  {m_tm n_tm : nat} :
...
#[global] Instance subst_tm_morphism2 {m_tm n_tm : nat} :
 (Proper (respectful (pointwise_relation _ eq) (pointwise_relation _ eq))
    (@subst_tm m_tm n_tm)).
Proof. exact (fun f_tm g_tm Eq_tm s => ext_tm f_tm g_tm Eq_tm s). Qed.

#[global] Instance ren_tm_morphism {m_tm n_tm : nat} :
 (Proper (respectful (pointwise_relation _ eq) (respectful eq eq))
    (@ren_tm m_tm n_tm)).
Proof.
  exact (fun f_tm g_tm Eq_tm s t Eq_st =>
...
#[global] Instance ren_tm_morphism2 {m_tm n_tm : nat} :
 (Proper (respectful (pointwise_relation _ eq) (pointwise_relation _ eq))
    (@ren_tm m_tm n_tm)).
